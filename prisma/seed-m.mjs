...
async function main() {
    ...
    for (const t of sampleTenants) {
        ...
        const adminRole = await prisma.role.create({ data: { name: "Admin", tenantId: tenant.id } });
        const facultyRole = await prisma.role.create({ data: { name: "Faculty", tenantId: tenant.id } });
        const studentRole = await prisma.role.create({ data: { name: "Student", tenantId: tenant.id } });
