    ...
    try {
      ...
      if (day.length === 2 && month.length === 2 && year.length === 4) {
        ...
        const maxDaysInMonth: { [key: number]: number } = {
          1: 31, 2: isLeapYear(parsedYear) ? 29 : 28, 3: 31, 4: 30, 5: 31, 6: 30,
          7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31,
        };

        // Auto-fix invalid dates
        if (parsedDay > maxDaysInMonth[parsedMonth]) {
          day = maxDaysInMonth[parsedMonth].toString();
          formattedValue = [day, month, year].join("-");
          setInputValue(formattedValue);
        }

        onChange(parse(formattedValue, "dd-MM-yyyy", new Date()));
      } else {
        onChange(null);
      }
    } catch (error) {
